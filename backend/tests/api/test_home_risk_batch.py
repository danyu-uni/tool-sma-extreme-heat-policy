...
def _forecast_json_for_successful_location(*, latitude: float, longitude: float) -> list[dict]:
    return [
        point.model_dump(mode="json")
        for point in _successful_risk_response(
            latitude=latitude,
            longitude=longitude,
        ).forecast
    ]
...
def test_post_home_risk_batch_success_returns_forecast_contract(
    profile: str,
    sport: str,
) -> None:
    """The batch route should serialize the dashboard forecast contract."""
        ...
        "locations": [
            {
                ...
                "timezone": "Australia/Sydney",
                "status": "ok",
                "forecast": _forecast_json_for_successful_location(
                    latitude=SYDNEY_LAT,
                    longitude=SYDNEY_LNG,
                ),
                "error_code": None,
...
def test_post_home_risk_batch_partial_failure_returns_200() -> None:
    ...
    assert body["locations"][0]["sport"] == "SOCCER"
...
def test_post_home_risk_batch_more_than_max_locations_returns_422() -> None:
    ...
    locations = [
        {
            "sport": "SOCCER",
            "latitude": -33.0 - (index * 0.01),
            "longitude": 151.0 + (index * 0.01),
        }
...
def test_post_home_risk_batch_invalid_sport_returns_422() -> None:
    """Each location should require official pythermalcomfort sport names."""
