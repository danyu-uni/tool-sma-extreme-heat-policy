...
def _expected_three_point_forecast_json() -> list[dict]:
    """Return the shared 3-point forecast JSON used by home and batch service tests."""
...
async def test_risk_service_uses_ttl_cache_for_same_input(
    ...
    assert first.model_dump(mode="json")["forecast"] == _expected_three_point_forecast_json()
...
async def test_risk_service_batch_cache_expires_after_ttl(
    ...
    payload = BatchRiskRequest(
        profile="ADULT",
        locations=[BatchRiskLocationRequest(sport="SOCCER", latitude=-33.847, longitude=151.067)],
    )
...
async def test_risk_service_batch_returns_summary_for_multiple_locations(
    ...
    response = await service.calculate_home_risk_batch(
        BatchRiskRequest(
            profile="ADULT",
            locations=[
                BatchRiskLocationRequest(sport="SOCCER", latitude=-33.847, longitude=151.067),
                BatchRiskLocationRequest(sport="SOCCER", latitude=-37.813, longitude=144.963),
            ],
        )
    )
    ...
    assert response.locations[0].model_dump(mode="json") == {
        ...
        "timezone": "Australia/Sydney",
        "status": "ok",
        "forecast": _expected_three_point_forecast_json(),
        "error_code": None,
        "detail": None,
    }
    assert response.locations[1].status == BatchRiskLocationStatus.OK
    assert response.locations[1].forecast is not None
    assert len(response.locations[1].forecast) == 3
    assert weather_client.calls == 1


async def test_risk_service_batch_supports_different_sports_per_location(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """Each batch location should use its own sport when building forecasts."""
    ...
    assert response.locations[0].sport == "SOCCER"
    assert response.locations[1].sport == "CROQUET"
    assert response.locations[0].status == BatchRiskLocationStatus.OK
    assert response.locations[1].status == BatchRiskLocationStatus.OK
    assert {payload.sport for payload in calculator.payloads} == {"SOCCER", "CROQUET"}
    assert all(payload.sport == "SOCCER" for payload in calculator.payloads[:3])
    assert all(payload.sport == "CROQUET" for payload in calculator.payloads[3:6])
...
async def test_risk_service_batch_cache_key_uses_home_coordinate_formatting(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """Batch cache keys should round coordinates to six decimals like `/home/risk`."""
    ...
    payload = BatchRiskRequest(
        profile="ADULT",
        locations=[
            BatchRiskLocationRequest(sport="SOCCER", latitude=first_latitude, longitude=longitude),
            BatchRiskLocationRequest(sport="SOCCER", latitude=second_latitude, longitude=longitude),
        ],
    )
