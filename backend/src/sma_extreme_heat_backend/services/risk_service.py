...
class RiskService:
    ...
    async def calculate_home_risk_batch(
        ...
        """Calculate dashboard forecasts for multiple locations in one request."""
    ...
    async def _fill_batch_cache_misses(
        ...
        *,
        ...
        """Fetch and compute uncached batch locations, storing successful results."""
        ...
        for index, (location, timezone_name, timezone_error), weather_result in zip(
            ...
            result = self._build_batch_location_result(
                location=location,
                timezone_name=timezone_name,
                timezone_error=timezone_error,
                weather_result=weather_result,
            )
            if result.status == BatchRiskLocationStatus.OK:
                self._batch_cache[
                    self._batch_cache_key(
                        sport=location.sport,
                        profile=payload.profile,
                ...
                ] = CacheEntry(value=result, expires_at=now + self.ttl_seconds)
    ...
    def _batch_cache_key(
        *,
        ...
        """Build a cache key for a successful dashboard location result."""

        return f"batch|{sport}|{profile}|{latitude:.6f}|{longitude:.6f}"
    ...
    def _build_batch_location_result(
        self,
        *,
        ...
        """Build one batch location result, mapping failures to per-location errors."""
        ...
        try:
            ...
            forecast = self._build_forecast(forecast_mrt_df=mrt_df, sport=location.sport)
